// Package controllersefile contains structs and queries for ALLRoles.
...
// Last Modified By:  
// Last Modified Date: 
// This api is to fetch the all ALLRoles.
...
// APIResponseALLRolesMaster defines the structure of the JSON response
type APIResponseALLRolesMaster struct {
	Status  int                        `json:"Status"`
	Message string                     `json:"message"`
	Data    []databaseefile.ALLRolesMaster `json:"Data"`
	PId     string                     `json:"P_id,omitempty"`
}

// ALLRolesMasterRequest defines the expected request structure
type ALLRolesMasterRequest struct {
	Data string `json:"Data"`
}

// ALLRolesDecryptedRequestData defines the structure of decrypted data
type ALLRolesDecryptedRequestData struct {
	Token string `json:"token"`
	PId   string `json:"P_id"`
} 

//ALLRoles handles the Bank Master API request.
func ALLRoles(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal encrypted request wrapper
	var decryptedData ALLRolesDecryptedRequestData
	...
	//Business logic - fetch ALLRoles master data
	data, err := databaseefile.GetALLRolesMaster()
	if err != nil {
	...
	response := APIResponseALLRolesMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
