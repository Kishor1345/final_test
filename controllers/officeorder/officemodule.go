...
type APIResponseOrderSubModule struct {
	Status  int         `json:"Status"`
	Message string      `json:"message"`
	Data    interface{} `json:"Data"`
}

type OrderSubModuleRequest struct {
	Data string `json:"Data"`
}

func OrderSubModule(w http.ResponseWriter, r *http.Request) {
	...
	auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// 5 Business logic
		data, totalCount, err := databaseofficeorder.GetOrderSubModuleFromDB(decryptedData)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}

		response := APIResponseOrderSubModule{
			Status:  200,
			Message: "Success",
