// Package  controllersqms contains structs and queries for quartermaster data fetch for approval.
//
...
// APIResponseQMSDataFetchForApprovalRequest defines
// encrypted API response structure for Quarters Master fetching data for approval
type APIResponseQMSDataFetchForApprovalRequest struct {
	Status  int         `json:"status"`
	Message string      `json:"message"`
	Data    interface{} `json:"data"`
}

// QMSDataFetchForApprovalRequest represents the encrypted  client request
// Expected format: "PID||EncryptedData"
type QMSDataFetchForApprovalRequest struct {
	Data string `json:"Data"`
}


// QuartersMasterDataFetchForApproval handles fetching data
func QuartersMasterDataFetchForApproval(w http.ResponseWriter, r *http.Request) {
	...
	var req QMSDataFetchForApprovalRequest
	...
	auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Fetch quarters Master  data for approval from database layer
		// Returns records list and total record count
		data, totalCount, err := databasequartersmasterestate.GetQuartersmasterFromDB(decryptedData)
		if err != nil {
		...
		// Response format:
		response := APIResponseQMSDataFetchForApprovalRequest{
			Status:  200,
			Message: "Success",
