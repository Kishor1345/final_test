// Package controllersqms contains structs and queries for quartermaster data fetch floor dropdown.
//
...
// APIResponseFloorDropdownRequest defines
// encrypted API response structure for quarters Master fetch for floor dropdown
type APIResponseFloorDropdownRequest struct {
	Status  int         `json:"status"`
	Message string      `json:"message"`
	Data    interface{} `json:"data"`
}

// FloorDropdownRequest represents the encrypted  client request
// Expected format: "PID||EncryptedData"
type FloorDropdownRequest struct {
	Data string `json:"Data"`
}

// QmesFloorDropdown handles fetching floor dropdown data
func QmesFloorDropdown(w http.ResponseWriter, r *http.Request) {
	...
	auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Fetch quaters Master floordropdown data from database layer
		// Returns records list and total record count
		data, totalCount, err := databasequartersmasterestate.QmesFloordatabase(decryptedData)
		if err != nil {
		...
		// Response format:
		response := APIResponseFloorDropdownRequest{
			Status:  200,
			Message: "Success",
