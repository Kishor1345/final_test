// Package controllersnoc contains data structures and database access logic for the NOC Approver page.
//
...
type APIResponseForNocApprover struct {
...
// Request structure
type NocApproverRequest struct {
	Data string `json:"Data"`
}


// Handler function
func NocApproverHandler(w http.ResponseWriter, r *http.Request) {
	...
	auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Call database layer
		data, totalCount, err := database.NocApproverDatabase(decryptedData)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}

		// UNWRAP JSONB data (VERY IMPORTANT)
		...
		response := APIResponseForNocApprover{
			Data:    responseData,
			P_id:    pid,
			Status:  200,
			Message: "Success",
