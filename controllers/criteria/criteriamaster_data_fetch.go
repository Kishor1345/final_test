//package controllerscriteria contains data structures and database access logic for the criteriamaster_data_fetch.
//
...
// APIResponseCriteriaMasterDataFetch defines
// encrypted API response structure for Criteria Master fetch
type APIResponseCriteriaMasterDataFetch struct {
...
// CriteriaMasterDataFetchRequest wraps encrypted client request
// Expected format: "PID||EncryptedData"
type CriteriaMasterDataFetchRequest struct {
	Data string `json:"Data"`
}

// CriteriaMasterDataFetch handles fetching Criteria Master data fetch
func CriteriaMasterDataFetch(w http.ResponseWriter, r *http.Request) {
	...
	var req CriteriaMasterDataFetchRequest
	...
	auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		data, totalCount,count, err := databasecriteria.CriteriaMasterDataFetchdatabase(decryptedData)
		if err != nil {
		...
		var Message string
		if count > 0 {
		Message = "Do You Want To Continue This Task?"
		}
		// Build success response with record count and data
		// Response format:
		response := APIResponseCriteriaMasterDataFetch{
			Status:  200,
			Message: "Success",
			Process:Message,
