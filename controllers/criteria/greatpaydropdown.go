//package controllerscriteria contains data structures and database access logic for the greatpaydropdown.
//
...
// APIResponseGreatPayDropdownRequest defines
// encrypted API response structure for Greatpay dropdown fetch
type APIResponseGreatPayDropdownRequest struct {
	Status  int         `json:"status"`
	Message string      `json:"message"`
	Data    interface{} `json:"data"`
}

// GreatPayDropdownRequest wraps encrypted client request
// Expected format: "PID||EncryptedData"
type GreatPayDropdownRequest struct {
	Data string `json:"Data"`
}

// GreatPayDropdown handles fetching dropdown data
func GreatPayDropdown(w http.ResponseWriter, r *http.Request) {
	...
	var req GreatPayDropdownRequest
	...
	auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Fetch Greatpaydropdown from database layer
		// Returns records list and total record count
		data, totalCount, err := databasecriteria.Greatpaydatabase(decryptedData)
		if err != nil {
		...
		// Response format:
		response := APIResponseGreatPayDropdownRequest{
			Status:  200,
			Message: "Success",
