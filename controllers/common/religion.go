// Package controllerscommon contains structs and queries for Religion.
...
// Last Modified By:  
// Last Modified Date: 
// This api is to fetch the all Religion.
...
// APIResponseReligionMaster defines the structure of the JSON response
type APIResponseReligionMaster struct {
...
// ReligionMasterRequest defines the expected request structure
type ReligionMasterRequest struct {
	Data string `json:"Data"`
}

// ReligionDecryptedRequestData defines the structure of decrypted data
type ReligionDecryptedRequestData struct {
	Token string `json:"token"`
	PId   string `json:"P_id"`
}

//Religion handles the Bank Master API request.
func Religion(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal encrypted request wrapper
	var decryptedData ReligionDecryptedRequestData
	...
	//Business logic - fetch religion master data
	data, err := databasesad.GetReligionMaster()
	if err != nil {
	...
	response := APIResponseReligionMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
