// Package controllerscommon contains data structures and database access logic for the EmployeeBasicInfo page.
//
...
type APIResponseForEmployeeBasicInfo struct {
...
type EmployeeBasicInfoRequest struct {
	Data string `json:"Data"`
}

func EmployeeBasicInfoHandler(w http.ResponseWriter, r *http.Request) {
	...
	var req EmployeeBasicInfoRequest
	...
	auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Call database layer
		data, totalCount, err := database.EmployeeBasicInfodatabase(decryptedData)
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}

		// Prepare the exact JSON structure for Postman
		responseData := map[string]interface{}{
			"No Of Records": totalCount,
			"Records":       data,
		}

		response := APIResponseForEmployeeBasicInfo{
			Status:  200,
			Message: "Success",
			P_id:    pid,
			Data:    responseData,
