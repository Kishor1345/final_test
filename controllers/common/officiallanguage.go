// Package controllerscommon contains structs and queries for Staff Additional details API.
...
// APIResponseOfficialLanguage defines the structure of the JSON response
type APIResponseOfficialLanguage struct {
...
// OfficialLanguageRequest defines the expected request structure
type OfficialLanguageRequest struct {
	Data string `json:"Data"`
}

// DecryptedRequestDatas defines the structure of decrypted data
type DecryptedRequestDatas struct {
	Token       string `json:"token"`
	PId         string `json:"P_id"`
	QuestionType string `json:"questiontype"`
}

//OfficialLanguage handles the Bank Master API request.
func OfficialLanguage(w http.ResponseWriter, r *http.Request) {
	...
	var req OfficialLanguageRequest
	...
	// Unmarshal encrypted request wrapper
	var decryptedData DecryptedRequestDatas
	...
	if decryptedData.QuestionType == "" {
		http.Error(w, "questiontype is required", http.StatusBadRequest)
		return
	}
	...
	//Business logic - fetch official language data with questiontype parameter
	data, err := databasesad.GetOfficialLanguage(decryptedData.QuestionType)
	if err != nil {
	...
	response := APIResponseOfficialLanguage{
		Status:  200,
		Message: "Success",
		Data:    data,
