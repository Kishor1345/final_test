// Package controllerscommon contains structs and queries for State.
...
// Last Modified By:  
// Last Modified Date: 
// This api is to fetch the all State.
...
// APIResponseStateMaster defines the structure of the JSON response
type APIResponseStateMaster struct {
...
// StateMasterRequest defines the expected request structure
type StateMasterRequest struct {
	Data string `json:"Data"`
}

// StateDecryptedRequestData defines the structure of decrypted data
type StateDecryptedRequestData struct {
	Token       string `json:"token"`
	PId         string `json:"P_id"`
	CountryCode string `json:"countrycode"`
}

//StateMaster handles the Bank Master API request.
func StateMaster(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal encrypted request wrapper
	var decryptedData StateDecryptedRequestData
	...
	//Business logic - fetch State master data with country code
	data, err := databasesad.GetStateMaster(decryptedData.CountryCode)
	if err != nil {
	...
	response := APIResponseStateMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
