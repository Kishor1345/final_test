// Package controllerscommon contains structs and queries for District.
...
// Last Modified By:  
// Last Modified Date: 
// This api is to fetch the all District.
...
// APIResponseDistrictMaster defines the structure of the JSON response
type APIResponseDistrictMaster struct {
...
// DistrictMasterRequest defines the expected request structure
type DistrictMasterRequest struct {
	Data string `json:"Data"`
}

// DistrictDecryptedRequestData defines the structure of decrypted data
type DistrictDecryptedRequestData struct {
...
//DistrictMaster handles the Bank Master API request.
func DistrictMaster(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal encrypted request wrapper
	var decryptedData DistrictDecryptedRequestData
	...
	if decryptedData.CountryCode == "" {
		http.Error(w, "Country code not found in decrypted data", http.StatusBadRequest)
		return
	}

	if decryptedData.StateID == 0 {
		http.Error(w, "State ID not found in decrypted data", http.StatusBadRequest)
		return
	}
	...
	//Business logic - fetch District master data with country code and state ID
	data, err := databasesad.GetDistrictMaster(decryptedData.CountryCode, decryptedData.StateID)
	if err != nil {
	...
	response := APIResponseDistrictMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
