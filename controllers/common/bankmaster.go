// Package controllerscommon contains structs and queries for Bank.
...
// Last Modified By:  
// Last Modified Date: 
// This api is to fetch the all Bank.
...
// APIResponseBankMaster defines the structure of the JSON response
type APIResponseBankMaster struct {
...
// BankMasterRequest defines the expected request structure
type BankMasterRequest struct {
	Data string `json:"Data"`
}

// BankDecryptedRequestData defines the structure of decrypted data
type BankDecryptedRequestData struct {
	Token string `json:"token"`
	PId   string `json:"P_id"`
}

//BankMaster handles the Bank Master API request.
func BankMaster(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal encrypted request wrapper
	var decryptedData BankDecryptedRequestData
	...
	data, err := databasesad.GetBankMaster()
	if err != nil {
	...
	response := APIResponseBankMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
