// Package controllerscommon contains structs and queries for City.
...
// Last Modified By:  
// Last Modified Date: 
// This api is to fetch the all City.
...
// APIResponseCityMaster defines the structure of the JSON response
type APIResponseCityMaster struct {
...
// CityMasterRequest defines the expected request structure
type CityMasterRequest struct {
	Data string `json:"Data"`
}

// CityDecryptedRequestData defines the structure of decrypted data
type CityDecryptedRequestData struct {
...
//CityMaster handles the Bank Master API request.
func CityMaster(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal encrypted request wrapper
	var decryptedData CityDecryptedRequestData
	...
	//Business logic - fetch City master data with country code and state ID
	data, err := databasesad.GetCityMaster(decryptedData.CountryCode, decryptedData.StateID)
	if err != nil {
	...
	response := APIResponseCityMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
