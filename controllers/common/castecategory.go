// Package controllerscommon contains structs and queries for castecagetory.
...
// Last Modified By:  
// Last Modified Date: 
// This api is to fetch the all castecagetory.
...
// APIResponseCasteCategoryMaster defines the structure of the JSON response
type APIResponseCasteCategoryMaster struct {
...
// CasteCategoryMasterRequest defines the expected request structure
type CasteCategoryMasterRequest struct {
	Data string `json:"Data"`
}

// CasteCategoryDecryptedRequestData defines the structure of decrypted data
type CasteCategoryDecryptedRequestData struct {
	Token string `json:"token"`
	PId   string `json:"P_id"`
}

//CasteCategory handles the Bank Master API request.
func CasteCategory(w http.ResponseWriter, r *http.Request) {
	...
	var req CasteCategoryMasterRequest
	...
	// Unmarshal encrypted request wrapper
	var decryptedData CasteCategoryDecryptedRequestData
	...
	//Business logic - fetch caste category master data
	data, err := databasesad.GetCasteCategoryMaster()
	if err != nil {
	...
	response := APIResponseCasteCategoryMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
