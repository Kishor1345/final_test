// Package controllerscommon contains structs and queries for languagemaster.
...
// APIResponseLanguageMaster defines the structure of the JSON response
type APIResponseLanguageMaster struct {
...
// LanguageMasterRequest defines the expected request structure
type LanguageMasterRequest struct {
	Data string `json:"Data"`
}

// LanguageMasterDecryptedRequestData defines the structure of decrypted data
type LanguageMasterDecryptedRequestData struct {
	Token string `json:"token"`
	PId   string `json:"P_id"`
}

//LanguageMaster handles the Bank Master API request.
func LanguageMaster(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal encrypted request wrapper
	var decryptedData LanguageMasterDecryptedRequestData
	...
	//Business logic - fetch language master data
	data, err := databasesad.GetLanguageMaster()
	if err != nil {
	...
	response := APIResponseLanguageMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
