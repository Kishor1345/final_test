// Package controllerscommon contains structs and queries for YearMaster.
...
// Last Modified By:  
// Last Modified Date: 
// This api is to fetch the all YearMaster.
...
// APIResponseYearMaster defines the structure of the JSON response
type APIResponseYearMaster struct {
...
// YearMasterRequest defines the expected request structure
type YearMasterRequest struct {
	Data string `json:"Data"`
}

// YearMasterDecryptedRequestData defines the structure of decrypted data
type YearMasterDecryptedRequestData struct {
	Token string `json:"token"`
	PId   string `json:"P_id"`
}

//Year handles the Bank Master API request.
func Year(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal encrypted request wrapper
	var decryptedData YearMasterDecryptedRequestData
	...
	//Business logic - fetch YearMaster data
	data, err := databasesad.GetYearMaster()
	if err != nil {
	...
	response := APIResponseYearMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
