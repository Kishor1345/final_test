// Package controllerscommon contains structs and queries for Country.
...
// Last Modified By:  
// Last Modified Date: 
// This api is to fetch the all Country.
...
// APIResponseCountryMaster defines the structure of the JSON response
type APIResponseCountryMaster struct {
...
// CountryMasterRequest defines the expected request structure
type CountryMasterRequest struct {
	Data string `json:"Data"`
}

// CountryDecryptedRequestData defines the structure of decrypted data
type CountryDecryptedRequestData struct {
	Token string `json:"token"`
	PId   string `json:"P_id"`
}

//CountryMaster handles the Bank Master API request.
func CountryMaster(w http.ResponseWriter, r *http.Request) {
	...
	// Unmarshal encrypted request wrapper
	var decryptedData CountryDecryptedRequestData
	...
	//Business logic - fetch Country master data
	data, err := databasesad.GetCountryMaster()
	if err != nil {
	...
	response := APIResponseCountryMaster{
		Status:  200,
		Message: "Success",
		Data:    data,
