// Package controllerslogin exposes API for SessionDecryptKey.
//
...
// APIResponseforSessionDecryptKey standard response
type APIResponseforSessionDecryptKey struct {
...
// Token wrapper
type SessionDecryptKeyTokenRequest struct {
	Token string `json:"token"`
}

// SessionDecryptKey API handler
func Datadecryptsessionkey(w http.ResponseWriter, r *http.Request) {
	...
	// Extract token
	var req SessionDecryptKeyTokenRequest
	...
	loggedHandler := auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// DB query
		data, total, err := databaselogin.SessionDecryptKeyDatabase(w, r)
		if err != nil {
		...
		// Build response
		resp := APIResponseforSessionDecryptKey{
			Status:  200,
			Message: "Success",
