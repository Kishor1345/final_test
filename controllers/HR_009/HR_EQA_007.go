...
package controllersQuarters
import (
	"Hrmodule/auth"
	databaseQuarters "Hrmodule/database/HR_009"
	"Hrmodule/utils"
	"encoding/json"
	"bytes"
	"fmt"
...
type APIResponseQuartersFetchForPreference struct {
...
type QuartersFetchForPreferenceRequest struct {
	Data string `json:"Data"`
}

// CriteriaMasterDataFetchForApproval handles fetching Criteria Master data for approval
func QuartersFetchForPreference(w http.ResponseWriter, r *http.Request) {
	...
	var req QuartersFetchForPreferenceRequest
	...
	auth.LogRequestInfo(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		data, totalCount, err := databaseQuarters.DatabaseQuartersPreference(decryptedData)
		if err != nil {
		...
		// Response format:
		response := APIResponseQuartersFetchForPreference{
			Status:  200,
			Message : "Quarters Preference Fetch Successfully",
