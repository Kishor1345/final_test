// Package modelscriteria contains structs and queries for criteria master fetch.
//
...
// Criteria represents individual criteria details
...
// CriteriaMasterDataFetchStructure represents
type CriteriaMasterDataFetchStructure struct{
...
var MyQueryCriteriaMasterDataFetch = `
...
`
// RetrieveOngoingCountForDataFetch checks whether
// any task is currently in ongoing status
func RetrieveOngoingCountForDataFetch(db *sql.DB) (int, error) {
	query := `
		SELECT COUNT(1)
		FROM meivan.cmes_m
		WHERE task_status_id = 22
	`
	var count int
...
// RetrieveCriteriaMasterDataFetch maps SQL rows
// into CriteriaMasterDataFetchStructure
func RetrieveCriteriaMasterDataFetch(rows *sql.Rows) ([]CriteriaMasterDataFetchStructure, error) {


	var results []CriteriaMasterDataFetchStructure

	for rows.Next() {
		var r CriteriaMasterDataFetchStructure
